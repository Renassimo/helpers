...
const FlightsTable = () => {
  const { flightsList } = useFlightsContext();
  const {
    filterQuery,
    setFilterQuery,
    visibleItems: visibleFlights,
  } = useFilter(flightsList, [
    'airline',
    'manufacturer',
    'model',
    'registration',
  ...
  const { paginationProps, rows: paginatedAndOrderedFlights } =
    useTablePagination(orderedFlights, [10, 25, 50, 100, flightsList.length]);
